...
const ProfileDialog: React.FC<ProfileDialogProps> = ({ open, onClose }) => {
    ...
    return (
        <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
            <DialogContent onOpenAutoFocus={focusFirstProfileField} closeLabel={t('close')} className="max-h-[85vh] overflow-y-auto scrollbar-thin sm:max-w-lg">
                ...
                <DialogDescription className="sr-only">{t('profileSection.personalData')}</DialogDescription>
