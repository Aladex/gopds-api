...
func TestRunAppliesEveryFileOnAnEmptyDatabase(t *testing.T) {
	...
	res, err := Run(context.Background(), db, fsys, "migrations", fresh)
	if err != nil {
...
func TestRunIsIdempotent(t *testing.T) {
	...
	if _, err := Run(context.Background(), db, fsys, "migrations", fresh); err != nil {
...
func TestRunAppliesOnlyWhatIsNew(t *testing.T) {
	...
	if _, err := Run(context.Background(), db, first, "migrations", fresh); err != nil {
		t.Fatalf("first run: %v", err)
	}
	...
	res, err := Run(context.Background(), db, both, "migrations", fresh)
	if err != nil {
		t.Fatalf("second run: %v", err)
	}
...
func TestRunBaselinesADatabaseThatPredatesTheLedger(t *testing.T) {
	...
	res, err := Run(context.Background(), db, fsys, "migrations", preexisting)
	if err != nil {
...
func TestRunRollsBackAFailedMigration(t *testing.T) {
	...
	_, err := Run(context.Background(), db, fsys, "migrations", fresh)
	if err == nil {
...
func TestPendingChangesNothing(t *testing.T) {
	...
	pending, baseline, err := Pending(ctx, db, fsys, "migrations", fresh)
	if err != nil {
		t.Fatalf("asking what is pending: %v", err)
	}
	if baseline {
		t.Error("an empty schema was reported as predating the ledger")
	}
	if len(pending) != 2 {
		t.Errorf("reported %v as pending, want both files", pending)
	}
...
func TestPendingShrinksAsMigrationsAreApplied(t *testing.T) {
	...
	if _, err := Run(ctx, db, files(map[string]string{"01-first.sql": "CREATE TABLE first (id int)"}),
		"migrations", fresh); err != nil {
		t.Fatalf("applying the first: %v", err)
	}

	pending, _, err := Pending(ctx, db, fsys, "migrations", fresh)
	if err != nil {
		t.Fatalf("asking what is pending: %v", err)
	}
	if len(pending) != 1 || pending[0] != "02-second.sql" {
		t.Errorf("reported %v as pending, want only the unapplied file", pending)
	}
}
